...
- 슬라이더 위치가 화면의 절대 X 좌표로 전송됩니다. `MOUSE_MODE_RELATIVE 1`로 바꾸면 마우스처럼 상대 이동을 보냅니다.
- **수동 센터 보정**: 게임이 커서를 가운데로 옮긴 직후 **Shift+Space를 동시에 1초 안에 3번** 누르면, 슬라이더가 가운데에서 떨어진 만큼을 상대 이동 한 번으로 보내 게임 커서와 슬라이더를 맞춥니다. LED가 두 번 깜빡이면 전송 완료.
...
- `MOUSE_MODE_RELATIVE`: 0 = 절대 좌표(기본), 1 = 상대 이동
