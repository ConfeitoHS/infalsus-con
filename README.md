...
- **수동 센터 보정**: 게임이 커서를 가운데로 옮긴 직후 **A S D F 네 개를 동시에, 0.5초 안에 4번** 누르면, 슬라이더가 가운데에서 떨어진 만큼을 상대 이동 한 번으로 보내 게임 커서와 슬라이더를 맞춥니다. LED가 두 번 깜빡이면 전송 완료.
...
- `RECENTER_CHORD_MASK`, `RECENTER_TAPS`, `RECENTER_WINDOW_MS`: 센터 보정 제스처 (ASDF / 4번 / 500ms)
